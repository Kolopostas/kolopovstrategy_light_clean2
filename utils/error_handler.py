...
class BybitAPIError(Exception):
    ...
    def __init__(self, message: str, ret_code: Optional[int] = None,
                 endpoint: Optional[str] = None, request_id: Optional[str] = None,
...
- Игнорирует 'неизменённые' коды (110043, 34040) как успех, если не указан raise_on_not_modified=True.
    ...
    if http_status == 429:
        ...
        logger.warning("[Bybit][RATE_LIMIT][HTTP429] %s endpoint=%s reqId=%s", msg, endpoint, request_id)
        raise BybitRateLimit(msg, ret_code=10006, endpoint=endpoint, request_id=request_id, payload=payload)
    ...
    if ret_code in code_to_ignore:
        ...
        logger.info("[Bybit][NOT_MODIFIED] retCode=%s msg=%s endpoint=%s", ret_code, ret_msg, endpoint)
        if raise_on_not_modified:
            raise BybitNotModified(ret_msg or "Not modified", ret_code=ret_code,
    ...
    if ret_code in _INVALID_PARAM_CODES:
        ...
        raise BybitInvalidParams(ret_msg or "Invalid parameters",
                                 ret_code=ret_code, endpoint=endpoint, request_id=request_id, payload=payload)

    if ret_code in _AUTH_CODES:
        raise BybitAuthError(ret_msg or "Auth/permission error",
                             ret_code=ret_code, endpoint=endpoint, request_id=request_id, payload=payload)

    if ret_code in _INSUFFICIENT_FUNDS_CODES:
        raise BybitInsufficientMargin(ret_msg or "Insufficient margin/balance",
                                      ret_code=ret_code, endpoint=endpoint, request_id=request_id, payload=payload)

    if is_retryable(ret_code):
        # Рекомендуется перехватывать и ретраить с экспон. бэк-оффом вне этого модуля
        raise BybitRateLimit(ret_msg or "Rate limited / temporary error",
                             ret_code=ret_code, endpoint=endpoint, request_id=request_id, payload=payload)

    # Частные коды из UTA/Trade, которые удобно подсветить явно
    if ret_code == 110009:
        raise BybitAPIError("TP/SL/conditional orders limit exceeded", ret_code=ret_code,
                            endpoint=endpoint, request_id=request_id, payload=payload)
    if ret_code == 110033:
        raise BybitAPIError("Can't set margin without an open position", ret_code=ret_code,
                            endpoint=endpoint, request_id=request_id, payload=payload)

    # Фоллбек
    message = ret_msg or f"Bybit error retCode={ret_code}"
    raise BybitAPIError(message, ret_code=ret_code, endpoint=endpoint, request_id=request_id, payload=payload)
...
def assert_bybit_ok(
    ...
    *,
    ...
    """
    ret_code, ret_msg = _normalize_ret_fields(response)
    logger.debug("[Bybit][RESP] retCode=%s retMsg=%s endpoint=%s reqId=%s",
                 ret_code, ret_msg, endpoint, request_id)
    handle_bybit_error(
response,
        endpoint=endpoint,
